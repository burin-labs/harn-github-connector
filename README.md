...
```sh
harn add github.com/burin-labs/harn-github-connector@v0.6.8
```
...
| Area | Methods |
|---|---|
| Pull requests | `github.pr.list`, `github.pr.create`, `github.pr.view`, `github.pr.edit`, `github.pr.upsert`, `github.pr.files`, `github.pr.commits`, `github.pr.checks`, `github.pr.merge`, `github.pr.enable_auto_merge`, `github.pr.disable_auto_merge`, `github.pr.comment`, `pulls.list`, `pulls.list_with_checks`, `pulls.get`, `pulls.update`, `pulls.create`, `pulls.merge`, `pulls.merge_safe`, `pulls.create_review_comment`, `pulls.get_diff`, `pulls.list_files`, `pulls.list_reviews`, `pull_requests.resolve_mergeable`, `repos.commit_pulls` |
| Actions and checks | `github.actions.workflow_dispatch`, `github.actions.runs`, `github.actions.run`, `github.actions.run_jobs`, `github.actions.run_cancel`, `github.actions.run_rerun`, `github.actions.logs`, `actions.workflow_dispatch`, `actions.workflow_runs.list`, `actions.workflow_run.get`, `actions.workflow_run.jobs`, `actions.workflow_run.cancel`, `actions.workflow_run.rerun`, `check_runs.create`, `check_runs.update` |
...
- `github.pr.upsert` creates or refreshes the sole open PR for an exact
  head/base pair and rejects stale or ambiguous heads before mutation.
- `github.pr.disable_auto_merge` requires exact pull-head and base-head leases
  plus the merge method needed for restoration. It compensates if either lease
  changes during GitHub's mutation.
- `github.branch.compare_trees` returns both commit and tree identities, so
  callers can distinguish equal content from equal revisions without parsing
  provider responses.
...
- `github.actions.workflow_dispatch` requires GitHub's returned run id and
  ...
  `github.actions.run_cancel` requests cancellation of one exact run, and
  `github.actions.run_rerun` requests a new attempt for one exact run;
...
| `actions_workflow_run_rerun(owner, repo, run_id, options)` | Request a new attempt for one exact workflow run. |
