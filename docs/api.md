...
pub type GithubConnectorError = {
  ...
  retryable?: bool,
  /**
   * The budget the refused response itself reported, present only when the
   * code is `rate_limited` and the response carried the headers.
   *
   * Absent is a fact, not a zero. A response with no rate-limit headers and a
   * response reporting nothing left are opposite situations for a caller
   * deciding whether to wait, and a field that defaulted to zero would make
   * the first read as the second.
   */
